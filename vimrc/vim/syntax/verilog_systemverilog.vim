...
syn keyword verilogStatement   trior trireg unsigned use vectored
...
"##########################################################
"       More Verilog + SystemVerilog Syntax
"##########################################################
...
syn keyword verilogLabel       join_any join_none fork join
...
" TODO Find a way to only highlight the keyword and the open/close braces. Not
" the string in between.
syn match   verilogDelay       "\<wait\>"
syn match   verilogDelay       "@"
...
syn keyword uvmMethod            get_type_name
" TLM ports
syn keyword uvmClass             uvm_tlm_b_initiator_socket uvm_tlm_b_target_socket b_transport
syn keyword uvmClass             semaphore uvm_tlm_time 
...
"##########################################################
"       Color Formatting :P TODO
"##########################################################

:hi Delays guifg=Red gui=bold guibg=black
...
:hi Gigolo guifg=Fuchsia guibg=black

" TODO These colors are not getting overridden from this syntax file. Check
" what the cause is and fix it.
...
if version >= 508 || !exists("did_verilog_syn_inits")
   ...
   "HiLink verilogStatement       Gigolo
   ...
   "HiLink uvmClass               Gigolo
   "HiLink uvmTypeDef             Gigolo
